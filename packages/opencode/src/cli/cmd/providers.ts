...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const selectedKey = args.provider
      ? options.find(
          (option) =>
    ...
    if (!selected && args.provider) return yield* fail(`Unknown configured provider "${args.provider}"`)
    if (!selected) return
    if (selected.auth === "wellknown") yield* Effect.orDie(authWellKnown.remove(selected.value))
    else yield* Effect.orDie(authSvc.remove(selected.value))
