...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const authWellKnown = yield* Effect.serviceOption(AuthWellKnown.Service)
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Option.isSome(authWellKnown)) {
          for (const item of yield* authWellKnown.value.configs().pipe(Effect.orDie)) {
            yield* merge(
              item.source,
...
export const node = LayerNode.make(layer, [FSUtil.node, Account.node, Env.node, Npm.node, httpClient])
